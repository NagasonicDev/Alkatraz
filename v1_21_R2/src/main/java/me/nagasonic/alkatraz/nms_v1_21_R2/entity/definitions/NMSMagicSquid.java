...
/**
 * Inert NMS scaffold for creating magic variants of {@code squid}. Identity and AI
 * come from core ({@link MagicBrains}) keyed by the stored {@link MagicEntityType}.
 */
public class NMSMagicSquid extends Squid implements MagicEntity {
    ...
    protected NMSMagicSquid(EntityType<? extends Squid> type, Level level, MagicEntityType magicType) {
        super(type, level);
    ...
    public static NMSMagicSquid spawn(MagicEntityType magicType, Location location) {
        ServerLevel level = ((CraftWorld) location.getWorld()).getHandle();

        NMSMagicSquid mob = new NMSMagicSquid(EntityType.SQUID, level, magicType);
