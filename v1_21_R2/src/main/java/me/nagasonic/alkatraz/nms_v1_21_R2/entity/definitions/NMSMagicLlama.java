...
/**
 * Inert NMS scaffold for creating magic variants of {@code llama}. Identity and AI
 * come from core ({@link MagicBrains}) keyed by the stored {@link MagicEntityType}.
 */
public class NMSMagicLlama extends Llama implements MagicEntity {
    ...
    protected NMSMagicLlama(EntityType<? extends Llama> type, Level level, MagicEntityType magicType) {
        super(type, level);
    ...
    public static NMSMagicLlama spawn(MagicEntityType magicType, Location location) {
        ServerLevel level = ((CraftWorld) location.getWorld()).getHandle();

        NMSMagicLlama mob = new NMSMagicLlama(EntityType.LLAMA, level, magicType);
