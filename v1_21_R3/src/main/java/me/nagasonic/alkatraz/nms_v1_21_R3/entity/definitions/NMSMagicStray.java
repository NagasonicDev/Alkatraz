...
/**
 * Inert NMS scaffold for creating magic variants of {@code stray}. Identity and AI
 * come from core ({@link MagicBrains}) keyed by the stored {@link MagicEntityType}.
 */
public class NMSMagicStray extends Stray implements MagicEntity {
    ...
    protected NMSMagicStray(EntityType<? extends Stray> type, Level level, MagicEntityType magicType) {
        super(type, level);
    ...
    public static NMSMagicStray spawn(MagicEntityType magicType, Location location) {
        ServerLevel level = ((CraftWorld) location.getWorld()).getHandle();

        NMSMagicStray mob = new NMSMagicStray(EntityType.STRAY, level, magicType);
