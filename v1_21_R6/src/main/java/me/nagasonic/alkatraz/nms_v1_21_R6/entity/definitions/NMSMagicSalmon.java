...
/**
 * Inert NMS scaffold for creating magic variants of {@code salmon}. Identity and AI
 * come from core ({@link MagicBrains}) keyed by the stored {@link MagicEntityType}.
 */
public class NMSMagicSalmon extends Salmon implements MagicEntity {
    ...
    protected NMSMagicSalmon(EntityType<? extends Salmon> type, Level level, MagicEntityType magicType) {
        super(type, level);
    ...
    public static NMSMagicSalmon spawn(MagicEntityType magicType, Location location) {
        ServerLevel level = ((CraftWorld) location.getWorld()).getHandle();

        NMSMagicSalmon mob = new NMSMagicSalmon(EntityType.SALMON, level, magicType);
